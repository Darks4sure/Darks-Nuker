...
class Darks:
    ...
    def menu(self):
        ...
        if ans in ["1", "01"]:
            ...
            Aizer = Aizer1 + ('\n @everyone `**DARKS NUKER | DARKS**` Runs Cord https://discord.gg/jrNNwggVm4  ')
            channels = self.session.get(f"https://discord.com/api/v9/guilds/{guildid}/channels", headers={"Authorization": f"Bot {token}"}).json()
...
@aizer.event
async def on_ready():
    print("{}({}Darks{}) Authenticated as{}: {}{}".format("\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", "\x1b[38;5;122m", "\x1b[0m", f"{aizer.user.name}#{aizer.user.discriminator}"))
    update_status.start()
